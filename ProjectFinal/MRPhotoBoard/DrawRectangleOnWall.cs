...
public class HandTrackingRaycaster : MonoBehaviour
{
    ...
    public LineRenderer lineRenderer;         // LineRenderer for ray visualization in Game view
    ...
    private void CastRayFromHand(Transform handAnchor)
    {
        ...
        if (Physics.Raycast(ray, out hitInfo, rayLength, wallLayer))
        {
            ...
            // Visualize the ray in Game view using LineRenderer
        ...
        {
            // If no hit, set the ray to the maximum length
            lineRenderer.SetPosition(0, ray.origin);
    ...
    private void PlaceHitMarker(Vector3 position)
    {
        ...
        foreach (var existingMarker in GameObject.FindGameObjectsWithTag("HitMarker"))
        {
            Destroy(existingMarker);
        }

        // Instantiate the hit marker at the collision point
        if (hitMarkerPrefab != null)
        {
            GameObject marker = Instantiate(hitMarkerPrefab, position, Quaternion.identity);
            marker.tag = "HitMarker"; // Tag it for cleanup
        }
    }
